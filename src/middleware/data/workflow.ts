...
export const upsertWorkflowVersion = async ({
  ...
  const { versionId, workflowId, version, status, versionData, recommended } = workflowVersion;
  const toUpsert: any = {
...
export const deleteAWorkflow = async ({
  ...
  const { data, error } = await supabase.from('workflow_version').delete().eq('id', workflowId);
